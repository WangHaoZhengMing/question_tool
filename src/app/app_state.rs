use crate::App;
use crate::app::LLMSettingsManager;
...
pub struct AppState {
    ...
    pub llm_settings: Arc<Mutex<LLMSettingsManager>>,
}
...
impl AppState {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // 创建 LLM 设置管理器
        let llm_settings = match LLMSettingsManager::new() {
            Ok(settings) => {
    ...
    fn setup_llm_provider_callback(&self, app: &App) {
        let settings: Arc<Mutex<LLMSettingsManager>> = self.llm_settings.clone();
        app.on_llm_provider_changed(move |provider| {
            if let Ok(mut settings) = settings.lock() {
                if let Err(e) = settings.set_provider(provider.to_string()) {
                    tracing::error!("[app_state] Failed to set provider: {}", e);
                }
            }
        });
    }
    ...
    fn setup_llm_model_callback(&self, app: &App) {
        ...
        app.on_llm_model_changed(move |model| {
            if let Ok(mut settings) = settings.lock() {
                if let Err(e) = settings.set_model(model.to_string()) {
                    tracing::error!("[app_state] Failed to set model: {}", e);
                }
            }
        });
    }
    ...
    fn setup_llm_api_key_callback(&self, app: &App) {
        ...
        app.on_llm_api_key_changed(move |api_key| {
            if let Ok(mut settings) = settings.lock() {
                if let Err(e) = settings.set_api_key(api_key.to_string()) {
                    tracing::error!("[app_state] Failed to set API key: {}", e);
                }
            }
        });
    }
    ...
    fn setup_llm_base_url_callback(&self, app: &App) {
        ...
        app.on_llm_base_url_changed(move |base_url| {
            if let Ok(mut settings) = settings.lock() {
                if let Err(e) = settings.set_base_url(base_url.to_string()) {
                    tracing::error!("[app_state] Failed to set base URL: {}", e);
                }
            }
        });
    }
    ...
    fn setup_llm_github_token_callback(&self, app: &App) {
        ...
        app.on_llm_github_token_changed(move |token| {
            if let Ok(mut settings) = settings.lock() {
                if let Err(e) = settings.set_github_token(token.to_string()) {
                    tracing::error!("[app_state] Failed to set GitHub token: {}", e);
                }
            }
        });
    }
    ...
    fn setup_llm_streaming_callback(&self, app: &App) {
        ...
        app.on_llm_streaming_changed(move |enabled| {
            if let Ok(mut settings) = settings.lock() {
                if let Err(e) = settings.set_streaming(enabled) {
                    tracing::error!("[app_state] Failed to set streaming: {}", e);
                }
            }
        });
    }
